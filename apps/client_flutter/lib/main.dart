...
class RecommendationFlowPage extends StatefulWidget {
  const RecommendationFlowPage({super.key});
...
class _RecommendationFlowPageState extends State<RecommendationFlowPage> {
  ...
  final _discovery = const MockDiscoveryService();
  late HouseholdScenario _scenario;
  late List<DetectedDevice> _devices;
  int _stepIndex = 0;

  @override
  void initState() {
    super.initState();
    _devices = _discovery.discoverVisibleDevices();
  ...
  Widget build(BuildContext context) {
    ...
    final steps = [
      _WelcomeStep(recommendation: recommendation),
      _DevicesStep(
        devices: _devices,
        onCategoryChanged: (device, category) {
          setState(() {
            final index = _devices.indexOf(device);
            _devices[index] = device.copyWith(category: category);
            _scenario = _scenario.copyWith(devices: List<DetectedDevice>.from(_devices));
          });
        },
      ),
      _UsageStep(
      ...
      _ResultsStep(recommendation: recommendation, scenario: _scenario),
    ];
...
class _WelcomeStep extends StatelessWidget {
  const _WelcomeStep({required this.recommendation});
...
class _ResultsStep extends StatelessWidget {
  const _ResultsStep({required this.recommendation, required this.scenario});
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('Your right-sized plan', style: theme.textTheme.headlineSmall),
        const SizedBox(height: 12),
        Text(
